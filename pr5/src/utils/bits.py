...
class UnsignedUnderflow(Exception):
    def __init__(self, data: int, width: int):
        m = unsigned_min()
        self.message = (
            f"Unsigned {data} less than minimum unsigned value {m} for width {width}."
        )
...
class UnsignedOverflow(Exception):
    def __init__(self, data: int, width: int):
        ...
        self.message = (
            f"Unsigned {data} more than maximum unsigned value {M} for width {width}."
        )
...
class SignedUnderflow(Exception):
    def __init__(self, data: int, width: int):
        m = signed_min(width)
        self.message = (
            f"Signed {data} less than minimum signed value {m} for width {width}."
        )
...
class SignedOverflow(Exception):
    def __init__(self, data: int, width: int):
        M = signed_max(width)
        self.message = (
            f"Signed {data} more than maximum signed value {M} for width {width}."
        )
